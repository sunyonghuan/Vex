...
| 导出 PDF |  | 导出图像型分页 PDF | 图片等资源嵌入 PDF 文件；导出成功后打开保存目录并定位文件；后续完善文本可选择和更精细分页 |
| 导出 Word(docx) |  | 导出 Word 文档 | 生成 `.docx`，保留基础 Markdown 结构、Word 样式和本地图片嵌入；导出成功后打开保存目录并定位文件 |
| 导出 PNG |  | 导出当前 Markdown 为长图 | 图片等资源随渲染结果写入 PNG，长文档按长图策略输出；导出成功后打开保存目录并定位文件 |
...
当前实现使用 OpenXML 包结构写入 Word 文档，支持基础标题、段落、列表、任务列表、引用、代码、分割线、表格、链接文本和本地图片嵌入。后续继续完善编号样式、复杂 HTML、分页控制、图片格式兼容和更细的标书格式要求。

图片等资源需要嵌入 Word 文件，通过邮件、QQ、微信等接收后也能正常预览格式、图片，Word 格式规范，正常标书要求格式。
...
94. 导出 HTML、PDF、PNG、Word 成功后会打开保存目录并定位文件；Word 导出已支持 `.docx` 样式和本地图片嵌入。
...
3. 修复后 Vex 预览、HTML/打印、PNG/PDF 图像型导出和复制路径中的 SVG 处理策略应保持一致；无法共享实现时必须记录差异。
...
6. PDF 导出成熟化：文本可选择，继续补齐更复杂块级元素的排版映射；任务列表状态和表格单元格 inline 样式已纳入图像型 PNG/PDF 导出。
