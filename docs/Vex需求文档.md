...
8. 后续预览区应支持类似 Typora 的可视化编辑：在 Markdown 渲染结果中直接编辑文本和常见结构，并回写 Markdown 源文本。

### 7.5 可视化编辑模式

1. CodeWF.Markdown 的 Markdown 控件需要增加“是否允许编辑”的开关，例如 `AllowEdit` 或等价属性。
2. 开启可视化编辑后，Markdown 控件应能在预览中编辑段落、标题、列表、引用、代码、表格等常见块，并向外发布 Markdown 文本变更。
3. Vex 默认以可视化编辑为主：中间源码编辑器隐藏，右侧 Markdown 区可直接编辑。
4. 用户可切换显示源码编辑器，实现“源码 + 可视化编辑”并存；两侧修改必须实时同步。
5. 可视化编辑不得破坏保存状态、撤销重做、查找替换、统计、大纲、导出和未保存确认流程。
6. 此能力复杂度较高，应在现有 MVVM 拆分、i18n、主题和基础编辑能力稳定后再集中实现。
